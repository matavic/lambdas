    ...
    try {
      switch (b[0]) {
        ...
        case 'watchlist':
          ...
          if(c[0] === 'i' && encontrado !== -1) {
            wm = await U.update({ _id: user._id, "watched.Title": it.Title }, { $pull: { watched: { Title:  it.Title } } });
            doc = {
              status: 'success',
              message: 'Register not modified'
            };  
        ...
        case 'votes':
          let oldvote = null;
          let oldvoteperson = null;
          let oldvoteposition = null;
          let resulting;
          let resulting2;
          ...
          if(encontrado !== -1) {
            ...
            oldvoteperson = user.votes[encontrado].person;
            oldvoteposition = user.votes[encontrado].position;
          }
          ...
          if(catVotes && catVotes.fnomineesdata.length > 0) {
            ...
            if(encontrado !== -1){
              ...
              // if(ind !== -1){
                ...
                resulting = await A.update( { award: it.award, categorytit: it.category }, { $set: { [`fnomineesdata.${ind}.nvotes`]: fixOldN, [`fnomineesdata.${ind}.pvotes`]: fixOldP } });
              // } else {
                // break;
              // }
            }
            // let newInd = catVotes.fnomineesdata.findIndex(o => o.Title === it.vote);
            let newInd = it.position;
            ...
            resulting2 = await A.update( { award: it.award, categorytit: it.category }, { $set: { [`fnomineesdata.${newInd}.nvotes`]: newN, [`fnomineesdata.${newInd}.pvotes`]: newP } });
            let newPercent;
            for(let i = 0; i < catVotes.fnomineesdata.length; i++){
              ...
              resp[i] = await A.update( { award: it.award, categorytit: it.category }, { $set: { [`fnomineesdata.${i}.pvotes`]: newPercent } });
            }
            const resArray = await Promise.all(resp);
          }
          re = c[0] === 'i' ? await U.update({ _id: user._id }, { $push: { votes: it } }) : await U.update({ _id: user._id, "votes.award": it.award, "votes.category": it.category }, { $set: { "votes.$.vote": it.vote, "votes.$.person": it.person, "votes.$.position": it.position, "votes.$.id": it.id } });
          break;        
        default:
          break;
      }
