...
async function run(a) {
  ...
  if (conn == null) {
    ...
    let resp = await doc.map( async function(f){
      ...
      for(let j=0; j < f.fnominees.length; j++) {
        let objnom = {};
        let objfnom = {};
        nomineesInfo.push(objnom);
        f.fnomineesdata.push(objfnom);
        try {
            movieSearch = await axios.get(apiTMDB + "/search/movie", {
              params: {
                // api_key: process.env.TMDB_API_KEY,
                api_key: 'c35c8fdfd00d784adc3b74f07f8f126e',
                query: f.fnominees[j],
            ...
            if(movieSearch.data.results.length > 0){ 
              // if(!nomineesInfo[j].Poster || nomineesInfo[j].Poster === "" || nomineesInfo[j].Poster === "N/A"){
                ...
                if(f.fwinner.length > 0 && Object.entries(f.winnerdata).length === 0 && (f.fwinner[0] === movieSearch.data.results[0].original_title || f.fwinner[0] === movieSearch.data.results[0].title)){
                  f.winnerdata.Title = movieSearch.data.results[0].original_title ? movieSearch.data.results[0].original_title : movieSearch.data.results[0].title;
                  f.winnerdata.Poster = posterBasePath + movieSearch.data.results[0].poster_path;
                  windata = true;
                }
              // } 
              ...
              // if((!nomineesInfo[j].Title || nomineesInfo[j].Title === "" || nomineesInfo[j].Title === "N/A") && movieSearch.data.results[0].title) {
                nomineesInfo[j].Title = movieSearch.data.results[0].original_title ? movieSearch.data.results[0].original_title : movieSearch.data.results[0].title;
                f.fnomineesdata[j].Title = movieSearch.data.results[0].original_title ? movieSearch.data.results[0].original_title : movieSearch.data.results[0].title;
              // }
              ...
              // if((!nomineesInfo[j].Actors || nomineesInfo[j].Actors === "" || nomineesInfo[j].Actors === "N/A" || !nomineesInfo[j].Director || nomineesInfo[j].Director === "" || nomineesInfo[j].Director === "N/A")) {
              ...
              if(movieTrailers.data.results.length > 0){
                ...
                youTube.setKey('AIzaSyBzKldOrcC_meggNEmy-YT97WJ0AsTMxhA');
      ...
      if(windata){
        let res2 = await M.update({ _id: f._id }, { fnomineesdata: f.fnomineesdata, winnerdata: f.winnerdata });
      } else {
            ...
            if(personDetails.data.known_for_department && (personDetails.data.known_for_department === "Acting" || personDetails.data.known_for_department === "Actors") && personCredits.data.cast.length > 0){
              ...
              let filmo = filmography.map(film => {
                return {
                  ...
                  Title: film.original_title ? film.original_title : film.title ? film.title : null,
